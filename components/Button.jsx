...
const types = {
	parent: {
		...
		'secondary-light': 'background-secondary-1 color-secondary-7 hover:color-secondary-8 hover:background-opacity-secondary-4',
		...
		'tertiary-light': 'background-tertiary-1 color-tertiary-7 hover:color-tertiary-8 hover:background-opacity-tertiary-4',
...
const Button = React.forwardRef(
	({ iconLeft, iconRight, children, style, size, type, onClick, className, disabled, loading, ...rest }, ref) => {
		...
		return (
			<button
				...
				type="submit"
				onClick={onClick}
				className={`border-radius-xs text-align-center transition-fast ${sizes.parent[size]} ${types.parent[finalType]} ${
			...
			>
				...
				>
					...
					{children}
					{!loading && iconRight && (
						<i className={`line-height-xxs ${iconRight} ${sizes.icon[size]} ${(children || iconLeft) && 'margin-left-xxs'}`} />
					)}
				</div>
			</button>
		);
